...
- **Before diagnosing any payment-write bug, check which GAS version is actually live:** `curl -s "$GOOGLE_APPS_SCRIPT_URL"` returns the version string. The repo shows what was *written*, never what is *deployed*. A stale deployment looks exactly like a logic error — this cost 3 months of silently failing payment edits (SYSTEM.md §14).
