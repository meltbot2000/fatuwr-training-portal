    ...
    <div className="bg-[#1E1E1E] rounded-2xl mb-3 overflow-hidden">
      ...
      >
        <div>
          ...
          <p className="text-[16px] text-[#888888] mt-0.5">{count} record{count !== 1 ? "s" : ""} · Total: {total}</p>
        </div>
        {open
      ...
      <main className="mx-auto max-w-[480px] px-4 py-4">
        <div className="flex justify-end mb-3">
          <button
            onClick={() => refetch()}
            disabled={isFetching}
            className="flex items-center gap-1.5 text-[16px] text-[#888888] hover:text-white transition-colors disabled:opacity-50"
          >
        ...
        {error && (
          <div className="py-12 text-center">
            ...
            <p className="text-[16px] text-[#888888]">{error.message}</p>
          </div>
        )}
          ...
          <div className="space-y-3">
            ...
            <div className="bg-[#1E1E1E] rounded-2xl divide-y divide-[#2C2C2C]">
              <div className="px-4 py-3 flex items-center justify-between">
                <div>
                  <p className="text-[16px] text-[#888888]">Club UEN (PayNow)</p>
                  <p className="text-[16px] font-medium text-white mt-0.5">{CLUB_UEN}</p>
                </div>
              ...
              <div className="px-4 py-3 flex items-center justify-between">
                <div>
                  <p className="text-[16px] text-[#888888]">Your payment ID</p>
                  <p className="text-[16px] font-medium text-white mt-0.5">{data.paymentId || "—"}</p>
                </div>
                {data.paymentId && <CopyButton value={data.paymentId} label="Payment ID" />}
              </div>

              <div className="px-4 py-3">
                <p className="text-[16px] text-[#888888] mb-0.5">Amount owed</p>
                {data.debt > 0 ? (
                  <p className="text-[18px] font-medium text-[#F5C518]">{formatFee(data.debt)}</p>
                ) : (
                  <div className="flex items-center gap-1.5">
                    <CheckCircle2 className="w-4 h-4 text-green-400" />
                    <p className="text-[16px] font-medium text-green-400">
            ...
            <div className="bg-[#1E1E1E] rounded-xl px-4 py-4">
              <p className="text-[16px] font-medium text-white mb-1">How to pay via PayNow</p>
            ...
            >
              {data.trainingFees.length === 0 ? (
                <p className="text-[16px] text-[#888888] text-center py-3">No training sign-ups yet.</p>
              ) : (
                <div className="divide-y divide-[#2C2C2C]">
                  {data.trainingFees.map((f, i) => (
                    <div key={i} className="flex items-center justify-between py-2.5">
                      <div>
                        <p className="text-[16px] text-white">{formatDate(f.trainingDate)}</p>
                        <p className="text-[16px] text-[#888888] mt-0.5">{f.pool}{f.pool && f.activity ? " · " : ""}{f.activity}</p>
                      </div>
                      <p className="text-[16px] font-medium text-white tabular-nums">{formatFee(f.actualFee)}</p>
                    </div>
                  ))}
                  <div className="flex items-center justify-between py-2.5">
                    <p className="text-[16px] font-medium text-white">Total</p>
                    <p className="text-[16px] font-medium text-white tabular-nums">{formatFee(data.totalTrainingFees)}</p>
                  </div>
                </div>
              )}
            </CollapsibleSection>
              ...
              >
                <div className="divide-y divide-[#2C2C2C]">
                  {data.membershipFees.map((f, i) => (
                    <div key={i} className="flex items-center justify-between py-2.5">
                      <div>
                        <p className="text-[16px] text-white">{f.activity}</p>
                        <p className="text-[16px] text-[#888888] mt-0.5">{formatDate(f.date)}</p>
                      </div>
                      <p className="text-[16px] font-medium text-white tabular-nums">{formatFee(f.actualFee)}</p>
                    </div>
                  ))}
                  <div className="flex items-center justify-between py-2.5">
                    <p className="text-[16px] font-medium text-white">Total</p>
                    <p className="text-[16px] font-medium text-white tabular-nums">{formatFee(data.totalMembershipFees)}</p>
                  </div>
                </div>
              </CollapsibleSection>
            )}
            ...
            >
              {data.payments.length === 0 ? (
                <p className="text-[16px] text-[#888888] text-center py-3">No payments recorded yet.</p>
              ) : (
                <div className="divide-y divide-[#2C2C2C]">
                  {data.payments.map((p, i) => (
                    <div key={i} className="flex items-center justify-between py-2.5">
                      <p className="text-[16px] text-[#888888]">{formatDate(p.date)}</p>
                      <p className="text-[16px] font-medium text-green-400 tabular-nums">{formatFee(p.amount)}</p>
                    </div>
                  ))}
                  <div className="flex items-center justify-between py-2.5">
                    <p className="text-[16px] font-medium text-white">Total</p>
                    <p className="text-[16px] font-medium text-green-400 tabular-nums">{formatFee(data.totalPaid)}</p>
                  </div>
                </div>
              )}
            </CollapsibleSection>
