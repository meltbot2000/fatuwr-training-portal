 ...
 *
 * v13 changes (2026-05-04):
 *   - editPaymentRow(): expanded to write ALL editable payment columns, not
 *     just col F (PaymentID) and col G (Email). Now also writes:
 *       col C (Date)      ← params.date
 *       col D (Amount)    ← params.amount
 *       col E (OTHR/Ref)  ← params.reference
 *     Previously, admin edits to date/amount/reference updated the DB but
 *     not the Sheet, so the next 6-hour sync or deployment startup sync
 *     (which does a full DELETE+INSERT from the Sheet) would silently revert
 *     those edits.
 *   - Server change (routers.ts): editPayment now throws a TRPCError if GAS
 *     fails (instead of falling through to update DB anyway). Admin sees the
 *     error and can retry. DB is only updated after Sheet write succeeds.
 *
 ...
 *   [1]  Subject                     (col B)
 *   [2]  Date                        (col C) ← editable via editPaymentRow v13
 *   [3]  Amount                      (col D) ← editable via editPaymentRow v13
 *   [4]  OTHR Message                (col E) ← editable via editPaymentRow v13
 *   [5]  PaymentID Match             (col F) ← GAS-resolved; editable
 *   [6]  Email                       (col G) ← GAS-resolved; editable
 */
...
function doGet(e) {
  return jsonResponse({ status: "ok", message: "FATUWR GAS v13 running" });
}
