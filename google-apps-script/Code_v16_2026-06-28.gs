 ...
 *
 * v16 changes (2026-06-28):
 *   - lookupUserByPaymentRef() now resolves the PayNow reference against the live
 *     Railway DB (GET /api/resolve-payment-ref) before falling back to the sheet
 *     User tab. Fixes payments not matching for users created since the last
 *     manual sheet sync (DB is source of truth for users). Sheet lookup remains
 *     as a fallback so behaviour never degrades if Railway is unreachable.
 *
...
function doGet(e) {
  return jsonResponse({ status: "ok", message: "FATUWR GAS v2 running" });
}
...
//
// processMaybankEmails() is called by a 1-min time-based trigger installed
// manually via the Apps Script editor (Triggers UI). It can also be run
...
/**
 * Find the user whose PaymentID case-insensitively matches `reference`.
 * Returns { paymentId, email } — both empty strings if no match found.
 *
 * v16 (2026-06-28): resolves against the live Railway DB first via
 * GET /api/resolve-payment-ref. The DB is the source of truth for users and is
 * always current; the sheet User tab only reflects the DB after a manual sync,
 * so any user created since the last sync was previously unmatchable here.
 * If the server is unreachable or returns no match, falls back to the local
 * sheet User tab (col A = PaymentID) — so behaviour never degrades below the
 * previous sheet-only logic.
 */
  ...
  try {
    ...
    if (url && secret) {
      ...
      if (resp.getResponseCode() === 200) {
        ...
        // 200 with no match → fall through to sheet (harmless double-check)
      } else {
...
// ─── Payment trigger ──────────────────────────────────────────────────────────
//
// processMaybankEmails() is driven by a 1-min time-based trigger installed
// manually via the Apps Script Triggers UI (clock icon). There is no helper
// function for creating it — it lives only in the Triggers UI.
//
// PREREQUISITE
// Gmail filters must apply the "Maybank" and/or "Maybank2" labels to incoming
// Maybank PayNow alert emails. processMaybankEmails() reads those labels.
...
/**
 * Fetches data for one tab from the Railway export endpoint and writes it to
 * the corresponding Sheet tab, replacing all existing data rows.
 *
...
function writeRowsToTab(tab, rows) {
  ...
  if (!rows || rows.length === 0) {
    Logger.log("[writeRowsToTab] " + tab + " — 0 rows, sheet cleared");
    return;
  }
