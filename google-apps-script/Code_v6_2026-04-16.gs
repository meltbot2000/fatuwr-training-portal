 ...
 *
 * v6 changes (2026-04-16):
 *   - writeRowsToTab: safety guard — never clears sheet data when DB returns 0 rows
 *     (prevents accidental data loss when DB tables are empty / not yet seeded)
 *
...
// matches a Gmail filter — typically within seconds of delivery.
//
// PREREQUISITE
// You must have Gmail filters that apply the "Maybank" and/or "Maybank2" labels
// to incoming Maybank PayNow alert emails.
//
 ...
 * Processes all threads in the Maybank and Maybank2 labels.
 */
function onNewPaymentEmail(e) {
  Logger.log("Gmail trigger fired — processing payment emails");
  try {
